...
colnames(expr)<-c("Control_for_Group2-1","Control_for_Group2-2",
                  ...
                  "Ythdc1_cKO_4OHT-1",
                  "Ythdc1_cKO_4OHT-2","Ythdc1_cKO_DMSO-1","Ythdc1_cKO_DMSO-2")
                  
#lib <- apply(expr,2,sum) # colculate library size
#cpm <- t(t(expr)/lib)*10^6 # colculate CPM

# 去除批次效应
#expr <- ComBat(dat = as.matrix(expr),batch=colnames(expr), mod=NULL, par.prior=TRUE)
...
DEG_merge<-c(rownames(sgDnmt1.vs.WT),rownames(sgMyc.vs.WT),
             ...
             rownames(Rif1_KO.vs.WT_pvalue0.05),rownames(RNF2_KD.vs.WT),
...
DEG_all_FC<-data.frame(sgDnmt1.vs.WT[DEG_all,"log2FoldChange"],
                       ...
                       Rif1_KO.vs.WT_pvalue0.05[DEG_all,"log2FoldChange"],
...
cor.dist(as.matrix(t(DEG_all_FC)))
library(pheatmap)
...
pheatmap(cor(DEG_all_FC),
         ...
         display_numbers = F,border=FALSE,
         filename = paste0(outDir,"/1rep.same.deg.cor.pdf"))

#install.packages("corrplot")
#library(corrplot)
#corrplot(cor(DEG_all_FC))
#corrplot(cor(DEG_all_FC), type = "lower", order = "hclust", tl.col = "black", tl.srt = 45,diag = F)
#corrplot(cor(DEG_all_FC), type = "upper", order = "hclust", tl.col = "black", tl.srt = 45,diag = F,col = colorRampPalette(c("yellow","red"))(100))


### 直接计算
#DEG_all_FC$SetdbKD<-log2((cpm[,"shSetdb1-1"]+10^-10)/(cpm[,"Setdb1WT-1"]+10^-10))[rownames(DEG_all_FC)]
#DEG_all_FC$Trim28KO<-log2((cpm[,"Trim28KO-1"]+10^-10)/(cpm[,"Trim28WT-1"]+10^-10))[rownames(DEG_all_FC)]
#cor.matrix<-cor.dist(as.matrix(t(DEG_all_FC)),diag=TRUE,upper=TRUE)
#aa<-as.matrix(cor.matrix)
#pheatmap(aa,
...
#         show_colnames=T,
#         display_numbers = T,
#         filename = paste0(outDir,"/all.coding_genes.cor.2.pdf"))
...
DEG_all_FC<-na.omit(DEG_all_FC)
write.csv(DEG_all_FC,file = "deg_fc_gene.csv")
...
#################################
# data in paper
DEG_paper<-c(rownames(shSae1.vs.WT_padj0.05),rownames(shSenp6.vs.WT_padj0.05),rownames(shSumo2.vs.WT_padj0.05),
             rownames(shUba2.vs.WT_padj0.05),rownames(shUbe2i.vs.WT_padj0.05),rownames(shTrim28.vs.WT_padj0.05),
             rownames(shChaf1a.vs.WT_padj0.05),rownames(shChaf1b.vs.WT_padj0.05)
)
...
cor.dist(as.matrix(DEG_paper_all_FC))
cor.matrix<-cor.dist(as.matrix(t(DEG_paper_all_FC)),diag=TRUE,upper=TRUE)
aa<-as.matrix(cor.matrix)
pheatmap(aa,
         color = colorRampPalette(c("red","yellow"))(100),
         show_rownames=T,
...
pdf(paste0(outDir,"/Venn_Rif1_RNF2.pdf"))
plot(vd,
...
plot(vd,
     cols = list(col = c("green","red"), alpha = 0.6),
     labels = list(col = "blue", font = 4), 
     edges = TRUE,
     quantities = TRUE
)
...
pdf(paste0(outDir,"/Venn_Rif1_Pcgf6.pdf"))
plot(vd,
     fills = list(fill = c(colors()[616], colors()[468],"red"), alpha = 0.6),
     labels = list(col = "white", font = 4), 
     edges = FALSE,
...
pdf(paste0(outDir,"/Venn_Rif1_Max.pdf"))
plot(vd,
     fills = list(fill = c(colors()[616], colors()[468],"red"), alpha = 0.6),
     labels = list(col = "white", font = 4), 
     edges = FALSE,
...
pdf(paste0(outDir,"/Venn_Rif1_Mga.pdf"))
plot(vd,
     fills = list(fill = c(colors()[616], colors()[468],"red"), alpha = 0.6),
     labels = list(col = "white", font = 4), 
     edges = FALSE,
...
myTABLE<-select(org.Mm.eg.db, keys=intersect(Rif1_KO_DEG,RNF2_KO_DEG),
                columns=c("SYMBOL","ENTREZID"), keytype="SYMBOL")

ego <- enrichGO(gene=myTABLE$ENTREZID,OrgDb='org.Mm.eg.db',keyType='ENTREZID',
                ont= "ALL",pAdjustMethod="BH",pvalueCutoff  = 0.05,qvalueCutoff  = 0.2,readable = T)
pdf(width=10,paste0(outDir,"/ego.barplot.pdf"))
barplot(ego, showCategory=15,title="Enrichment GO") #条形图
dev.off()
pdf(width=10,paste0(outDir,"/ego.dotplot.pdf"))
dotplot(ego, showCategory=15,title="Enrichment GO")
dev.off()
write.xlsx(as.data.frame(ego),file = paste0(outDir,"/ego.xlsx"),
           sheetName="ego",col.names = T,row.names = F,append = F)


ekegg <- enrichKEGG(unique(myTABLE$ENTREZID), organism="mmu", keyType = "ncbi-geneid",
                    pvalueCutoff=0.05,pAdjustMethod = "BH",qvalueCutoff = 0.2)
pdf(paste0(outDir,"/ekegg.barplot.pdf"))
barplot(ekegg, showCategory=15,title="Enrichment KEGG") #条形图
dev.off()
pdf(paste0(outDir,"/ekegg.dotplot.pdf"))
dotplot(ekegg, showCategory=15,title="Enrichment KEGG")
dev.off()
write.xlsx(as.data.frame(ekegg),file = paste0(outDir,"/ekegg.xlsx"),
           sheetName="ekegg",col.names = T,row.names = F,append = F)