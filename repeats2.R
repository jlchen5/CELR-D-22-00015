...
colnames(count) <- c("RepName",filelist)
head(count)


colnames(count) <- gsub(pattern="repeats/",replacement = "",colnames(count))
colnames(count) <- gsub(pattern=".repeats.count.simple",replacement = "",colnames(count))
head(count)

rownames(count) <- count$RepName
...
write.csv(DEG_all_FC,file = "rep_fc.csv")
